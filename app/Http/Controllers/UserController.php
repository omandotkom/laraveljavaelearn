...
class UserController extends Controller
{
    ...
    public function deleteInstructor($id)
    {
        $user = User::findOrFail($id);
        //ambil kelas
        $class = Kelas::where('user_id', $user->id)->first();
        if (is_null($class)) {
            return abort("403", "Instruktur ini tidak memiliki kelas.");
        }
        $ungraduate = DB::select("select count(1) as total from userclasses join users on userclasses.user_id = users.id where userclasses.class_id =1 and users.status != 'graduate'");
        if(is_null($ungraduate)){
            return abort("403", "Instruktur ini tidak memiliki siswa.");
        }
        if ($ungraduate[0]->total > 0){
            return abort('403','Instruktur ini memiliki siswa yang belum lulus.');
        }
        else{
            $user->delete();
        }
