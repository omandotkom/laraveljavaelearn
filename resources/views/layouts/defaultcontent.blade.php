<div class="col-md-4">
    <div class="card">
        <div class="card-body">
            <div class="d-flex">
                <h4 class="card-title">Tentang</h4>
            </div>
            <div class="d-flex align-items-center flex-row mt-30">
                <img src="{{url('dashboardasset/img/info.png')}}" class="rounded img-fluid mx-auto d-block" alt="...">
            </div>
            <div class="d-flex align-items-center flex-row mt-30">
                <a type="button" role="button" href="{{route('about')}}" class="btn mx-auto btn-outline-info"><i class="ik ik-info"></i>Tentang Kami</a>
            </div>
        </div>
    </div>
</div>
<div class="col-md-4">
    <div class="card">
        <div class="card-body">
            <div class="d-flex">
                <h4 class="card-title">Sejarah Java</h4>
            </div>
            <div class="d-flex align-items-center flex-row mt-30">
                <img src="{{url('dashboardasset/img/info.png')}}" class="rounded img-fluid mx-auto d-block" alt="...">
            </div>
            <div class="d-flex align-items-center flex-row mt-30">
                <a type="button" role="button" href="{{route('history')}}" class="btn mx-auto btn-outline-info"><i class="ik ik-info"></i>Lihat Sejarah</a>
            </div>
        </div>
    </div>
</div>